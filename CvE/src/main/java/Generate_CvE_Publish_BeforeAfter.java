...
public class Generate_CvE_Publish_BeforeAfter
{
	static Date ES_FROM;
	static Date ES_TO;
	...
	static String RESULT_FILE = "result3.tsv";
	static String RESULT_PUBLISH_FILE = "result_publish3.tsv";
	static Interval INTERVAL = Interval.DAY;

	static CVE_ES_Connector ES = new CVE_ES_Connector("localhost", 9300, "cve",
			"logs");
	static CVEXmlReader CVER = new CVEXmlReader();
	static SimpleDateFormat SDF = new SimpleDateFormat("yyyy-MM-dd HH:mm:ssZ");
	static SimpleDateFormat SDFT = new SimpleDateFormat(
	...
	static List<String> CVE_DATE_LIST = new ArrayList<String>();
	static ArrayList<JSONObject> CVE_DETAIL;
	static ArrayList<ArrayList<String>> CVE_BF_List = new ArrayList<ArrayList<String>>();
	...
	private static void setCVE_LIST()
	{
		...
		for (JSONObject cve : CVE_DETAIL)
		{
			...
			CVE_DATE_LIST.add(cve.getString("published_date"));
		}
	...
	private static void setDATE_LIST()
	{
		...
		DATE_LIST = ES.getDateList("@timestamp", ES_FROM.getTime(),
				ES_TO.getTime(), INTERVAL);
		DATE_LIST_DATE = ES.getDateListDate("@timestamp", ES_FROM.getTime(),
				ES_TO.getTime(), INTERVAL);
	...
	{
		int count = 0;
		long from, to, gapbefore = 345600000, gapafter = 431999999;
		SDF_DATE.setTimeZone(TimeZone.getTimeZone("GMT"));
		SDF.setTimeZone(TimeZone.getTimeZone("GMT"));
		for (int i = 0; i < CVE_DETAIL.size(); i++)
		{
			HashMap<String, Long> dateCountMap = new HashMap<String, Long>();
			CVE_BF_List.add(new ArrayList<String>());
			String cve_id = CVE_DETAIL.get(i).getString("cve_id");
			from = CVER.String_To_Date(CVE_DATE_LIST.get(i)).getTime()
					- gapbefore;
			to = CVER.String_To_Date(CVE_DATE_LIST.get(i)).getTime() + gapafter;
			System.out.println(SDF_DATE.format(CVER
					.String_To_Date(CVE_DATE_LIST.get(i))));
			System.out.println(SDF.format((new Date(from))));
			System.out.println(SDF.format((new Date(to))));
			DateHistogram result = ES.getDateDistribution("@timestamp", from,
					to, "text:\"" + cve_id + "\"", INTERVAL);
			// count+=result.getBuckets().size();
			for (Bucket b : result.getBuckets())
			{
				count += b.getDocCount();
				CVE_BF_List.get(CVE_BF_List.size() - 1).add(
						b.getKeyAsText().string());
	...
	private static void dumpResult() throws IOException
	{
		...
		writer.write("date\t-4\t-3\t-2\t-1\tpublished\t1\t2\t3\t4" + "\n");

		for (Entry<String, HashMap<String, Long>> entry : CVE_DATE_MAP
				.entrySet())
		{
			...
			for (int j = 0; j < CVE_BF_List.get(i).size(); j++)
			{
				DateString = entry.getValue().get(CVE_BF_List.get(i).get(j));
				if (DateString != null)
				{
					System.out.println(DateString);
					tmp.add(DateString.toString());
				}
			}
	...
	private static void dumpResultPublishedDate() throws IOException
	{
		...
		for (JSONObject cve : CVE_DETAIL)
		{
			...
			for (Date date : DATE_LIST_DATE)
			{
				...
				{
					Date pdate = SDF_DATE
							.parse(cve.getString("published_date"));
